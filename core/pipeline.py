...
class PipelineState(TypedDict):
    """Internal state for the LangGraph pipeline."""
    user_input: str
    source: str
    reply: str
    tasks: List[Dict]
...
class AEGISPipeline:
    """
    LangGraph wrapper around the existing core pipeline.
    This replaces the sequential logic in HumanInterface with a formal graph.
    """
    def __init__(self, llm_engine, speech_manager, event_bus, task_manager, learning_manager=None):
        ...
        # Load configuration for switches
        self.config = {}
    ...
    async def _node_planner(self, state: PipelineState) -> PipelineState:
        """Analyzes user input. Phase C: Uses AutoGen team if enabled."""
        # Voice interactions need low latency; avoid an extra LLM "planning" pass by default.
        if self.use_autogen and state.get("source") != "voice":
            ...
            # The team only returns a text-based plan or intent string
            # We don't let it execute anything!
            state["metadata"]["autogen_plan"] = plan_data
            
            # Extract simple intents for task/search from the plan if present
        ...
        # Fallback to legacy planner logic (Phase B)
    ...
    async def _run_autogen_team(self, user_input: str) -> str:
        """Runs a real planning pass without executing tools."""
        if not self.llm:
            return ""
    ...
    def _build_prompt(self, state: PipelineState) -> str:
        # Include Autogen plan in context if available
