...
SATURDAY_MQTT_SERVER = "192.168.0.180"
SATURDAY_MQTT_PORT = 1884

BG = 0x000000
SATURDAY_BLUE = 0x00AAFF
SATURDAY_GREEN = 0x00FF88
SATURDAY_ORANGE = 0xFF8800
SATURDAY_RED = 0xFF4444
WHITE = 0xFFFFFF
CYAN = 0x00FFFF
...
def init_display():
    ...
    try:
        ...
        lcd.clear(BG)
        lcd.rect(0, 0, 320, 30, SATURDAY_BLUE, BG)
        lcd.setFont(lcd.FONT_DejaVu24)
        lcd.setColor(WHITE)
        lcd.textCenter(160, 5, "SATURDAY HOMEBOT")
        lcd.rect(0, 215, 320, 25, SATURDAY_BLUE, BG)
...
def update_display(wifi_ok, mqtt_ok, ip=""):
    if not HAS_M5STACK: return
    try:
        lcd.rect(10, 40, 90, 25, SATURDAY_GREEN if wifi_ok else SATURDAY_RED, BG)
        lcd.setFont(lcd.FONT_DejaVu16)
        lcd.setColor(BG)
        lcd.text(15, 45, "WiFi:" + ("ON" if wifi_ok else "OFF"))
        
        lcd.rect(110, 40, 90, 25, SATURDAY_GREEN if mqtt_ok else SATURDAY_ORANGE, BG)
        lcd.setColor(BG)
...
def draw_eye(frame, direction_cmd):
    ...
    try:
        ...
        lcd.circle(x, y, r + 3 + int(pulse), SATURDAY_BLUE)
        lcd.circle(x, y, r, SATURDAY_BLUE)
        ...
        if direction_cmd == "FWD": px, py = x, y - 15
            ...
            if blink > 0.95:
                lcd.line(x - 20, y, x + 20, y, SATURDAY_BLUE)
                return
            px, py = x, y
        
        lcd.circle(px, py, 15, SATURDAY_BLUE)
        lcd.circle(px, py, 8, 0x000011)
        lcd.circle(px - 4, py - 4, 4, WHITE)
        
        if direction_cmd not in ["STOP"]:
            if direction_cmd == "FWD":
                lcd.line(x, y - 80, x - 15, y - 60, SATURDAY_GREEN)
                lcd.line(x, y - 80, x + 15, y - 60, SATURDAY_GREEN)
            elif direction_cmd == "REV":
                lcd.line(x, y + 80, x - 15, y + 60, SATURDAY_RED)
                lcd.line(x, y + 80, x + 15, y + 60, SATURDAY_RED)
    except: pass
...
def show_direction(direction_cmd):
    ...
    try:
        ...
        lcd.setFont(lcd.FONT_DejaVu16)
        if direction_cmd == "STOP": lcd.setColor(SATURDAY_RED)
        elif direction_cmd in ["RTL", "RTR"]: lcd.setColor(SATURDAY_ORANGE)
        else: lcd.setColor(SATURDAY_GREEN)
...
def show_battery():
    ...
    try:
        ...
        try:
            ...
            color = SATURDAY_GREEN if bat_pct > 50 else SATURDAY_ORANGE if bat_pct > 20 else SATURDAY_RED
            lcd.setColor(color)
...
def show_loading():
    ...
    try:
        lcd.setFont(lcd.FONT_DejaVu24)
        lcd.setColor(SATURDAY_BLUE)
        lcd.textCenter(160, 100, "LOADING...")
        for i in range(3):
            lcd.circle(130 + i * 25, 130, 5, SATURDAY_BLUE)
            time.sleep_ms(200)
    except: pass
...
def mqtt_connect():
    ...
    try:
        mqtt_client = MQTTClient("SATURDAY_HB", SATURDAY_MQTT_SERVER, SATURDAY_MQTT_PORT)
        mqtt_client.set_callback(mqtt_callback)
        mqtt_client.connect()
        mqtt_client.subscribe("saturday/homebot/cmd")
        connected_mqtt = True
        print("MQTT OK")
        mqtt_pub("ONLINE")
        return True
...
def mqtt_pub(msg):
    ...
    if connected_mqtt and mqtt_client:
        try:
            mqtt_client.publish("saturday/homebot/status", msg)
        except:
            connected_mqtt = False
