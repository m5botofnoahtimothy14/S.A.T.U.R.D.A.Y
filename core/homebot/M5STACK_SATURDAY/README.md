# SATURDAY HomeBot - M5Stack CORE2
## Complete Firmware v3.0

### Features
- **WiFi Connection** - Connects to your network
- **MQTT** - Communicates with SATURDAY server
- **Display UI** - Shows status, IP, battery, direction
- **SATURDAY Eye Animation** - Animated eye that looks in movement direction
...
#### 3. Set SATURDAY Server IP
```python
SATURDAY_MQTT_SERVER = "192.168.0.100"  # Your SATURDAY PC IP
```
...
```
+--------------------------------+
|     SATURDAY HOMEBOT              |  <- Title
|                                |
|   WiFi: OK    MQTT: OK         |  <- Status
|                                |
|        ( SATURDAY EYE )           |  <- Animated eye
...
### MQTT Topics
- Subscribe: `saturday/homebot/cmd` - Receive commands from SATURDAY
- Publish: `saturday/homebot/status` - Send status to SATURDAY
...
4. MQTT not connecting? - Check SATURDAY MQTT server IP

### SATURDAY Integration
When connected to SATURDAY, you can say:
- "HomeBot move forward"
- "HomeBot turn left"
- "HomeBot stop"
SATURDAY will send the command via MQTT to the HomeBot!
