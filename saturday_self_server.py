...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "saturday_server.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("SATURDAY.Server")
class SATURDAYServer:
    def __init__(self):
        self.process = None
        self.running = False
    ...
    def start_saturday(self):
        ...
        logger.info(f"SATURDAY Core started with PID: {self.process.pid}")
        return True
    def stop_saturday(self):
        if self.process:
            logger.info("Stopping SATURDAY Core...")
            try:
                self.process.terminate()
                self.process.wait(timeout=10)
            except subprocess.TimeoutExpired:
                self.process.kill()
            logger.info("SATURDAY Core stopped")
    def monitor_loop(self):
        while self.running:
            if self.process:
                retcode = self.process.poll()
                if retcode is not None:
                    logger.warning(f"SATURDAY Core exited with code: {retcode}")
                    if self.restart_count < self.max_restarts:
                        self.restart_count += 1
                        logger.info(f"Restarting SATURDAY... (attempt {self.restart_count})")
    ...
    def run(self):
        ...
        logger.info("="*50)
        logger.info("SATURDAY SELF-OWNED SERVER STARTING")
        logger.info("="*50)
